#include<3dp_localization/3dp_localization.h>

#include <boost/timer.hpp>


//using namespace amcl;		
Pcld_localization::Pcld_localization(ros::NodeHandle nh):nh_(nh),
...
void Pcld_localization::localization(){}
void Pcld_localization::load_gloabalmap(const string &globalmap_pcd_dir)
{
    //std::string globalmap_pcd_dir = private_nh_.param<std::string>("globalmap_pcd", "");
...
void Pcld_localization::generate_refmap(const Eigen::Isometry3d &cameraPose)
{
...
void Pcld_localization::set_currmap(const PointCloud::Ptr currCloud,const Eigen::Isometry3d &cameraPose)
{
...
int main( int argc, char** argv )
{
	...
	if(readimgfromfile(image_database_dir,215,color_img,depth_img,camera_pose))
	{
		...
		Pcld_localization pcld_loc(nodehandle);
